...
public final class ForceField implements Listener {
	...
	public void updateViewedBlocks(PlayerMoveEvent e) {
		if (!ConfigWorldGuard.OPTION_FORCEFIELD_ENABLED) return;
		Player p = e.getPlayer();
		if(!Combat.isInCombat(p)) return;
		Location t = e.getTo();
		Location f = e.getFrom();
		if (t.getBlockX() == f.getBlockX() && t.getBlockY() == f.getBlockY() && t.getBlockZ() == f.getBlockZ()) return;
				Set<Location> changedBlocks = getChangedBlocks(p);
				Set<Location> removeBlocks = new HashSet<>();
				if (previousUpdates.containsKey(p.getUniqueId())) removeBlocks = previousUpdates.get(p.getUniqueId());
				for (Location location : changedBlocks) {
					p.sendBlockChange(location, Material.getMaterial(WorldGuardUtil.getMaterial(ConfigWorldGuard.OPTION_FORCEFIELD_MATERIAL)), WorldGuardUtil.getData(ConfigWorldGuard.OPTION_FORCEFIELD_MATERIAL));
	...
	private Set<Location> getChangedBlocks(Player p) {
		...
		if (!Combat.isInCombat(p)) return locations;
		...
		for (int x = bottomBlockX; x <= topBlockX; x++) {
			for (int z = bottomBlockZ; z <= topBlockZ; z++) {
				...
				if (WorldGuardUtil.isSafeZone(location)) continue;
				...
				for (int i = -radius; i < radius; i++) {
					...
					loc.setY(loc.getY() + i);
					if (!(loc.getBlock().getType().equals(Material.AIR) || loc.getBlock().getType().equals(Material.STATIONARY_WATER) || loc.getBlock().getType().equals(Material.WATER) || loc.getBlock().getType().equals(Material.STATIONARY_LAVA) || loc.getBlock().getType().equals(Material.LAVA))) continue;
